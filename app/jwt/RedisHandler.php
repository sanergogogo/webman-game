...
class RedisHandler
{
    ...
    public static function verifyToken(string $pre, string $uid, string $token): bool
    {
        ...
        if (!Redis::exists($cacheKey)) {
            throw new JwtCacheTokenException('该账号已在其他设备登录，强制下线', ErrorCode::JWTCacheTokenReplace);
        }
        if (Redis::get($cacheKey) != $token) {
            throw new JwtCacheTokenException('身份验证令牌已失效', ErrorCode::JWTCacheTokenInvalid);
        }
        return true;
    }
