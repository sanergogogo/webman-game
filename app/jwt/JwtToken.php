...
class JwtToken
{
    ...
    public static function refreshToken(): array
    {
        ...
        try {
            ...
            throw new JwtTokenException('刷新令牌无效', ErrorCode::JWTRefreshTokenSignatureInvalid);
        } catch (BeforeValidException $beforeValidException) {
            throw new JwtTokenException('刷新令牌尚未生效', ErrorCode::JWTRefreshTokenBeforeValid);
        } catch (ExpiredException $expiredException) {
            throw new JwtTokenException('刷新令牌会话已过期，请再次登录！', ErrorCode::JWTRefreshTokenExpired);
        } catch (UnexpectedValueException $unexpectedValueException) {
            throw new JwtTokenException('刷新令牌获取的扩展字段不存在', ErrorCode::JWTRefreshTokenUnexpectedValue);
        } catch (\Exception $exception) {
            throw new JwtTokenException($exception->getMessage(), ErrorCode::JWTRefreshTokenInvalid);
        }
    ...
    {
        if (!isset($extend['id'])) {
            throw new JwtTokenException('缺少全局唯一字段：id', ErrorCode::JWTRefreshTokenUnexpectedValue);
        }
        $config = self::_getConfig();
        ...
        $token = [
            'token_type' => 'Bearer',
            'expired_at' => time() + $config['access_exp'],
    ...
    public static function verify(int $tokenType = self::ACCESS_TOKEN, string $token = null): array
    {
        ...
        try {
            ...
            throw new JwtTokenException('身份验证令牌无效', ErrorCode::JWTTokenSignatureInvalid);
        } catch (BeforeValidException $beforeValidException) {
            throw new JwtTokenException('身份验证令牌尚未生效', ErrorCode::JWTTokenBeforeValid);
        } catch (ExpiredException $expiredException) {
            throw new JwtTokenException('身份验证会话已过期，请重新登录！', ErrorCode::JWTTokenExpired);
        } catch (UnexpectedValueException $unexpectedValueException) {
            throw new JwtTokenException('获取的扩展字段不存在', ErrorCode::JWTTokenUnexpectedValue);
        } catch (\Exception $exception) {
            throw new JwtTokenException($exception->getMessage(), ErrorCode::JWTTokenInvalid);
        }
    }
    ...
    private static function getTokenFromHeaders(): string
    {
        ...
        if (!$authorization || 'undefined' == $authorization) {
            throw new JwtTokenException('请求未携带authorization信息', ErrorCode::JWTTokenNoAuthorization);
        }

        if (self::REFRESH_TOKEN != substr_count($authorization, '.')) {
            throw new JwtTokenException('非法的authorization信息', ErrorCode::JWTTokenAuthorizationError);
        }

        if (2 != count(explode(' ', $authorization))) {
            throw new JwtTokenException('Bearer验证中的凭证格式有误，中间必须有个空格', ErrorCode::JWTTokenAuthorizationError);
        }

        [$type, $token] = explode(' ', $authorization);
        if ('Bearer' !== $type) {
            throw new JwtTokenException('接口认证方式需为Bearer', ErrorCode::JWTTokenAuthorizationError);
        }
        if (!$token || 'undefined' === $token) {
            throw new JwtTokenException('尝试获取的Authorization信息不存在', ErrorCode::JWTTokenNoAuthorization);
        }
    ...
    private static function _getConfig(): array
    {
        $config = config('jwt');
        if (empty($config)) {
            throw new JwtTokenException('jwt配置文件不存在', ErrorCode::JWTInternalError);
        }
        return $config;
    }
